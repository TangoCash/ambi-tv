   ...
   int b)
{
   ...
   struct ambitv_uart_priv* uart =
	...
	if (idx >= ambitv_special_sinkcommand_brightness)
	{
		...
		case ambitv_special_sinkcommand_gamma_blue:
		{
			...
			if (g)
				ret = uart->gamma[idx] * 100;
			else
			{
				uart->gamma[idx] = (double) r / 100.0;
				tptr = uart->gamma_lut[idx];
				uart->gamma_lut[idx] = ambitv_color_gamma_lookup_table_create(uart->gamma[idx]);
				ambitv_color_gamma_lookup_table_free(tptr);
				ambitv_log(ambitv_log_info, LOGNAME "gamma-%s was set to %.2f", scol[idx], uart->gamma[idx]);
				ret = 0;
			}
		}
			break;
		...
		case ambitv_special_sinkcommand_intensity_blue:
		{
			...
			if (g)
				ret = uart->intensity[idx];
			else
			{
				uart->intensity[idx] = r / 100;
		...
		case ambitv_special_sinkcommand_min_intensity_blue:
		{
			...
			if (g)
				ret = uart->intensity_min[idx];
			else
			{
				uart->intensity_min[idx] = r / 100;
				ambitv_log(ambitv_log_info, LOGNAME "intensity-min-%s was set to %d%%", scol[idx],
						uart->intensity_min[idx]);
				ret = 0;
			}
		}
			break;
   ...
   if (NULL != outp) {
      ...
      for (i=0; i<3; i++) {
		*rgb[i] =
				(unsigned char) (((((int) *rgb[i] * uart->brightness) / 100) * uart->intensity[i]) / 100);
         if (uart->gamma_lut[i])
            *rgb[i] = ambitv_color_map_with_lut(uart->gamma_lut[i], *rgb[i]);
      }

      if(r < uart->intensity_min[0]*2.55) r = uart->intensity_min[0]*2.55;
      if(g < uart->intensity_min[1]*2.55) g = uart->intensity_min[1]*2.55;
      if(b < uart->intensity_min[2]*2.55) b = uart->intensity_min[2]*2.55;
...
ambitv_uart_print_configuration(struct ambitv_sink_component* component)
{
   ...
   ambitv_log(ambitv_log_info,
      ...
      "\tgamma (rgb):       %.2f, %.2f, %.2f\n",
         ...
         uart->brightness,
         uart->intensity[0], uart->intensity[1], uart->intensity[2],
         uart->intensity_min[0], uart->intensity_min[1], uart->intensity_min[2],
